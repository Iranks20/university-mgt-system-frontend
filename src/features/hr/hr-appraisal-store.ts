...
function buildTemplateApiPayload(
  ...
  return {
    ...
    definition: {
      ...
      ...(template.formCapture ? { formCapture: template.formCapture } : {}),
    },
  };
}
...
export async function updateAppraisalCycleStatus(
  id: string,
  status: HrAppraisalCycle['status']
): Promise<HrAppraisalCycle> {
  return hrAppraisalService.updateCycleStatus(id, status);
}
...
export async function getTeamAppraisalReviews(): Promise<HrAppraisalReview[]> {
  return hrAppraisalService.getTeamReviews();
}
