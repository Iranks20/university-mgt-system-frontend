...
export default function ManagementOverview() {
  ...
  const pendingTotal =
    (overview?.pendingCancellations ?? 0) + (overview?.pendingSubstitutions ?? 0);

  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-gray-900">University Overview</h1>
        <p className="text-gray-500">Executive summary of institutional performance and risk signals.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <CardContent className="pt-6">
              ...
              <div>
                ...
                <h3 className="text-3xl font-bold">
                  {loading ? (
                    ...
                    (overview?.totalEnrolment?.toLocaleString() ?? '—')
                  )}
                </h3>
              </div>
            ...
            <div className="mt-4">
              <Button asChild variant="link" className="px-0 h-auto text-sm">
                <Link to="/management-enrolment">Enrolment health</Link>
              </Button>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-l-4 border-l-blue-600">
          <CardContent className="pt-6">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-sm font-medium text-gray-500">Teaching rate (30d)</p>
                <h3 className="text-3xl font-bold">
                  {loading ? (
                    <Loader2 className="h-8 w-8 animate-spin text-gray-400" />
                  ) : overview?.teachingRate != null ? (
                    `${overview.teachingRate.toFixed(1)}%`
                  ) : (
                    '—'
                  )}
                </h3>
            ...
            <div className="mt-4 text-sm text-gray-500">
              Untaught: {overview?.untaughtCount30d ?? '—'}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-l-4 border-l-red-500">
          <CardContent className="pt-6">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-sm font-medium text-gray-500">Open disruptions</p>
                <h3 className="text-3xl font-bold">
                  {loading ? (
                    <Loader2 className="h-8 w-8 animate-spin text-gray-400" />
                  ) : (
              ...
              <CalendarX className="h-8 w-8 text-red-500 opacity-20" />
            </div>
            <div className="mt-4">
              <Button asChild variant="link" className="px-0 h-auto text-sm">
                <Link to="/cancellations">Review queue</Link>
              </Button>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Button asChild variant="outline" className="justify-start h-auto py-4">
          <Link to="/management-risk">
            <AlertTriangle className="h-4 w-4 mr-2 text-amber-600" />
            Risk register
          </Link>
        </Button>
        <Button asChild variant="outline" className="justify-start h-auto py-4">
          <Link to="/management-lecturer-performance">
            <BarChart className="h-4 w-4 mr-2 text-[#015F2B]" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Staff presence today</CardTitle>
            <CardDescription>Checked-in staff as a share of active staff.</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="text-4xl font-bold">
              {loading ? (
                <Loader2 className="h-8 w-8 animate-spin text-gray-400" />
              ) : overview?.staffPresent != null ? (
                `${overview.staffPresent}%`
              ) : (
                '—'
              )}
            </div>
            <p className="mt-3 text-sm text-gray-500">
              Active courses in catalogue: {overview?.activeCourses?.toLocaleString() ?? '—'}
            </p>
            <Button asChild variant="outline" size="sm" className="mt-6">
              <Link to="/management-staff-performance">Staff performance</Link>
            </Button>
          </CardContent>
        </Card>
      </div>
